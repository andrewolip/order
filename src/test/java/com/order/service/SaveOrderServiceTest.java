        ...
        when(repository.save(any(Order.class)))
                .thenReturn(
                        new Order(
                            ...
                            STATUS,
                            Set.of(
                              new Product(UUID.randomUUID().toString(), PRODUCT, BigDecimal.valueOf(250.50), 1)), ZonedDateTime.now()
                            )
                );

        var order = service.save(new OrderDto(CUSTOMER, STATUS, Set.of(new ProductDto(PRODUCT, CATEGORY, BigDecimal.valueOf(250.50), 150))));
